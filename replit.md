...
-   **Employer Onboarding**: Multi-state hiring selection (all 50 states + territories) during employer creation. Auto-generated unique questionnaire URLs per employer. Welcome email sent automatically with links to Form 9198 signing and engagement letter via employer onboarding wizard. QR code generation for mobile questionnaire access.
-   **Email Notifications**: Transactional emails for screening invites, status updates, invoices, and employer welcome emails (with Form 9198 and engagement letter links) via Resend.
...
-   **New Hire Onboarding Module**: Opt-in per employer (admin toggle). Mobile-first digital onboarding portal (`/onboard/:token`) for W-4, state withholding, direct deposit, emergency contact, ID upload, and policy e-signatures. Employer management page with invite creation, progress tracking, and completion metrics. Dashboard card shows upcoming hires and readiness status. Phase 2: Email/SMS notifications, instance detail view (tasks/forms/docs/timeline with masked PII), reminder worker (day 3 & 7), bulk CSV import with flexible column mapping, analytics (funnel/status/time-to-complete charts). Phase 3: Employer onboarding settings (customize required/optional/disabled steps, deadline days, welcome message), onboarding templates (per-department configs with step overrides), completion actions (auto-create employee record, optional WOTC screening trigger), document export (JSON download of completed forms/docs), and admin onboarding overview (cross-employer stats, daily trend, employer breakdown charts). Schema: `onboarding_invite_tokens`, `onboarding_instances`, `onboarding_tasks`, `onboarding_documents`, `onboarding_form_data`, `onboarding_settings`, `onboarding_templates`. Routes: `server/routes/onboarding.ts` (employer), `server/routes/publicOnboarding.ts` (public). Admin: `/admin/onboarding`.
